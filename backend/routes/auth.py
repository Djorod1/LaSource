...
def _envoyer_email_verification(id_user: int, email: str, prenom: str):
    ...
    try:
        ...
        lien = request.host_url.rstrip("/") + "/verifier-email.html?jeton=" + jeton
        envoyer(
            email,
            "Bienvenue sur LaSource — confirmez votre adresse",
            f"Bonjour {prenom},\n\n"
            f"Bienvenue sur LaSource. Pour activer votre compte,\n"
...
def connexion():
    ...
    user = recuperer_un(
        """SELECT id_utilisateur, mot_de_passe, role, est_actif, email_verifie
...
def verifier_email():
    ...
    expire = ligne["expire_le"]
    if isinstance(expire, str):
        try:
            expire = datetime.fromisoformat(expire)
        except ValueError:
            return _erreur("Jeton expiré.", 410)
